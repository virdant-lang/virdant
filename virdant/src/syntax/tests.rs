...
mod test_clock_domain {
    ...
    #[test]
    fn test_parse_dependson() {
        let source = test_source(
            b"mod Foo {\n    outgoing clk_out : Clock {\n        it dependson clk_in\n    }\n}\n"
        );
        let parsing = parse(&source);
        assert!(parsing.errors().is_empty());
        // Verify the AST contains a ModDefStmtDependsOn node.
        let root = parsing.ast_node(parsing.root().children()[0].id());
        let moddef = &root.children()[0];
        let it_block = &moddef.children()[1];
        let block = &it_block.children()[0];
        let stmt = &block.children()[0];
        assert!(
            matches!(stmt.payload(), AstNodePayload::ModDefStmtDependsOn),
            "Expected ModDefStmtDependsOn, got {:?}",
            stmt.payload()
        );
    }

    #[test]
    fn test_parse_dependson_standalone() {
        let source = test_source(
            b"ext mod Foo {\n    incoming clk_in : Clock\n    outgoing clk_out : Clock\n    clk_out dependson clk_in\n}\n"
        );
        let parsing = parse(&source);
        for err in parsing.errors() {
            eprintln!("Parse error: {:?}", err);
        }
        // Skip dump in the test - use direct AST access.
        let root = parsing.ast_node(parsing.root().children()[0].id());
        // root is the ModDef node; its children are the statements.
        let stmt = &root.children()[2]; // third child: clk_out dependson clk_in
        assert!(
            matches!(stmt.payload(), AstNodePayload::ModDefStmtDependsOn),
            "Expected ModDefStmtDependsOn, got {:?}",
            stmt.payload()

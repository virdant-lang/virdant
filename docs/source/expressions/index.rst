...
This means a statement such as `counter <= counter + 1` is taking the previous value of `counter`, incrementing it,
...
Operators
---------
Virdant supports many common operators:
...
Any and all
-----------
Given an expression `e` of type `Word[n]`, you can reduce by OR'ing together all the bits using `any(e)`
or AND'ing them using `all(e)`.
The result is has type `Bit`.

Casting
-------
Given an expression `e` of an enum type, you can cast it to a `Word[n]` using `cast(e)`.


Indexing
--------
You can statically index into a word with the syntax `w[0]`, `w[1]`, ..., etc.

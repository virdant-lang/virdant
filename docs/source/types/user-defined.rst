...
However, this has the disadvantage that designers have to keep track of the color encoding in their heads.
...
Union Types
===========
A `union type` is what is sometimes known as an algebraic data type.
...
Here is an example of a definition for a union type:
...
You construct values of a union type by calling the constructors, preceded by an `@` sign.
...
* `@Done(result)` introduces `result`, which also has type `Word[8]`.
...
* If the state is running, subtract the larger of `x` and `y` from the other, or halt if `y` is zero.
...
Each enum type is declared with an explicit `width`, indicating the number of bits needed to represent each value.

...
And they allow you to design your hardware in a hierarchical fashion.

Each Virdant project consists of a number of module definitions.
...
The keyword `it` is an alias for the component being defined.
Here, `it` refers to `queue`.
It is optional, but it can often make our code easier to read.

The first driver, `it <= inp`, will latch the value of `inp` into the register `queue` on every clock cycle.

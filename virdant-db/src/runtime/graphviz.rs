//! GraphViz dependency graph export

use std::fmt::Write;
use indexmap::{IndexMap, IndexSet};
use crate::runtime::{Db, QueryTrait, QueryResultTrait};
...
where
    ...
    R: QueryResultTrait,
{
    /// Export the query dependency graph to a GraphViz DOT file
    pub fn save_graphviz<P: AsRef<std::path::Path>>(&self, filepath: P) -> std::io::Result<()> {
        let map = self.map.lock().unwrap();

        let mut queries: Vec<Q> = map
            .keys()
            .cloned()
        ...
        let query_indices: IndexMap<Q, usize> = queries
            .iter()
            .cloned()
            .enumerate()
            .map(|(i, query)| (query, i))
